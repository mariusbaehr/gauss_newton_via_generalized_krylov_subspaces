...
def compare():
    ...
    plt.figure(figsize=(8, 4), dpi=300)
    plt.plot(range(1,len(gn_data[3])+1),gn_data[3], "-s", label="Gauß-Newton")
...
def compare_without_scaling():
    ...
    plt.plot(range(1,len(gn_data[2])+1),gn_data[2], "-s", label="Gauß-Newton")
    plt.plot(range(1,len(gnk_data[2])+1),gnk_data[2], "-x", label="GNK")
    plt.plot(range(1,len(gnk_ii_data[2])+1),gnk_ii_data[2], "-+", label="GNK-(II)")
    plt.plot(range(1,len(ref_data[2])+1),ref_data[2], ".-", label="Referenz")
    plt.gca().set_xticks([1,10,20,30,40,50,60,70,80])
...
def compare_linear():
    ...
    plt.plot(range(1,len(gnk_ii_data[2])+1),gnk_ii_data[2], "-+", label="GNK-(II)")
    plt.plot(range(1,len(ref_data[2])+1),ref_data[2], ".-", label="Referenz")
    plt.plot(range(1,len(cg_data[2])+1),cg_data[2], "-", label="CG")
    plt.gca().set_xticks([1,20,40,60,80,100])
    plt.gca().yaxis.set_major_locator(ticker.MaxNLocator(integer=True))
    plt.xlabel("Iterationen")
    plt.ylabel(r"Anzahl Residuums Auswertungen")
    plt.legend()
