...
#    )
#    benchmark(
#        res, x0, jac, error, {"args": (H,y_true_h,alpha,lamb), "max_iter": 100, "tol": 1e-12}, title="bratu_"
#    )
...
#    
#    benchmark(
#        res, x0, jac, error, {"args": (H,y_manufactured_solution,alpha ,lamb), "max_iter": 22, "tol": 1e-12}, title="bratu_manufacture_")
#    
...
#        res, x0, jac, error, {"args": (H,y_noise_h, alpha, lamb), "max_iter": 100, "tol": 1e-12}, title="bratu_noise_")
    ...
    def aTa(x,h, rhs, alpha, lamb):
        return (h**-2*A2 + h**-1*alpha * Dx1).T @ ((h**-2*A2 + h**-1 * alpha * Dx1) @ x)
    
    ATA1 = scipy.sparse.linalg.LinearOperator(((N - 1) ** 2, (N - 1) ** 2), matvec= lambda x:aTa(x,1,None,0,0))
    
    def ref_cg(callback):
        cg, _ = scipy.sparse.linalg.cg(
            ATA1,x0, callback=callback
        )
        return cg
