...
def res(x): return y_true-A_h2@x-a1*D_x1@x-a2*np.exp(x)
...
def jac(x): return -A_h2-a1*D_x1-a2*scipy.sparse.diags(np.exp(x))
def error(x): return np.linalg.norm(x_true-x)
...
def ref_cg(callback):
    cg, _ = scipy.sparse.linalg.cg(A_TA, A_h2.T@y_true+a1*D_x1.T@y_true, x0, callback=callback)
    return cg

benchmark(res,x0, jac, error, {'max_iter':300, 'tol':1E-12}, title="bratu_pde")#,additional_methods=[ref_cg])
