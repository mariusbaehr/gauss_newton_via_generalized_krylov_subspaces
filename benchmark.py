...
def benchmark(res, x0, jac, error, kwargs={}, additional_methods=[], title=None):
    ...
    def callback_gn(x, nfev, cg_iter):
        global error_list, loss_list, nfev_list, cg_iter_list
        error_list.append(error(x))
        loss_list.append(loss(x))
        nfev_list.append(nfev)
    ...
    for method in methods:
        ...
        ax4.plot(range(len(cg_iter_list)), cg_iter_list, "x-", label=method.__name__)
    ...
    if not title == None:
        ...
        fig4.savefig(title + "cg_iter" + ".png", bbox_inches="tight")
