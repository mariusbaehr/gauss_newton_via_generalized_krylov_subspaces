...
def res(x,tau): return np.array([x[0]+1,tau*x[0]**2+x[0]-1])
def jac(x,tau): return np.array([[1], [2*tau*x[0] +1]])
def loss(x,tau): return np.sum(res(x,tau)**2)
x0 = np.array([1.0])
...
tau = -5
x_exact = np.array([0]) 
ax[0].plot(x_exact,loss(x_exact,tau),"ks",label = r"$x^\ast$", zorder=-1, markersize=7)
def error_list(x_list): return np.abs(x_exact-x_list)
x_min=0
x_max=0
...
linestyles = ["-","-", "-."]
markers = ["o","+","x"]
for step_length_control, color, linestyle, marker in zip(step_length_controls,colors,linestyles, markers):
    x_list = [x0]
    gauss_newton(res,x0,jac, args = (tau,), max_iter = max_iter, callback=cb_x, step_length_control=step_length_control)
    ax[0].scatter(x_list, [loss(np.array([x]),tau) for x in x_list], marker=marker , color=color)

    ax[1].semilogy(range(len(x_list)),error_list(x_list), marker=marker, linestyle=linestyle, label = step_length_control.__name__, color=color)
    x_min = min(np.min(x_list),x_min)
    x_max = max(np.max(x_list),x_max)

x_span = np.linspace(x_min,x_max,100)
ax[0].plot(x_span, [loss(np.array([x]),tau) for x in x_span], color="black", zorder=-1, label = r"$\mathcal{L}$")

for k in range(10):
    if k == 3 or k == 2:
        continue
    ax[0].text(x_list[k],loss(np.array([x_list[k]]),tau),str(k),ha="left", va="bottom", color="tab:orange", fontsize=10)
ax[0].text(x_list[3],loss(np.array([x_list[3]]),tau),str(3),ha="right", va="bottom", color="tab:orange", fontsize=10)
ax[0].text(x_list[2],loss(np.array([x_list[2]]),tau),str(2),ha="right", va="bottom", color="tab:orange", fontsize=10)
