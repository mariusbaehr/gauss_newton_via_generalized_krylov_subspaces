...
def jac(x):
    block1 = 10*scipy.sparse.eye(p-1,p,k=1) - 20*scipy.sparse.diags(x[:-1],shape=(p-1,p))
    block2 = -scipy.sparse.eye(p-1,p,k=0)
    return scipy.sparse.block_array([[block1],[block2]]).todense() #todense() to ensure the classical gauss-newton method is used, with direct least squares solver.
